...
void setup() {
  ...
  logo.resize(int(0.9 * width), 0);
  background = loadImage("ArkanoidFondo.png");
...
void draw() {
  ...
  background(0);
  imageMode(CENTER);
  image(background, 0.5 * width, 0.5 * height);
  ...
  if (status == 0) {
    ...
    textSize(60);
    imageMode(CENTER);
    image(logo, 0.5 * width, 0.5 * height);
